...
FROM c_pp pp, c_mn mn, c_in2 in2, c_hh hh, amd_temp_pc pclist WHERE pclist.store_uid = $store_uid and pclist.POSTCODE = pp.POSTCODE and pclist.POSTCODE = mn.POSTCODE and pclist.POSTCODE = in2.POSTCODE and pclist.POSTCODE = hh.POSTCODE ";
...
$query = "SELECT SUM(seg.HH_TOT) as HH_TOT, seg.CLUSTER as CLUSTER, seg.POSTCODE, pclist.POSTCODE FROM c_seg seg , amd_temp_pc pclist where seg.POSTCODE = pclist.POSTCODE and pclist.store_uid = $store_uid group by CLUSTER  ";
if ($result = mysqli_query($link, $query)) {
while ($row = mysqli_fetch_assoc($result)) {
...
// updated august 28 2018 c_geo -> amfd_homes_current
/*
$query = "SELECT SUM(c_geo.homes) as mfd_home, SUM(c_geo.apts) as mfd_apts, SUM(c_geo.farm) as mfd_farm FROM c_geo c_geo, amd_temp_pc pclist WHERE c_geo.PostCode = pclist.POSTCODE and pclist.store_uid = $store_uid";
if ($result = mysqli_query($link, $query)) {
while ($row = mysqli_fetch_assoc($result)) {
...
$query = "SELECT SUM(amfd_homes_current.homes) as mfd_home, SUM(amfd_homes_current.apts) as mfd_apts, SUM(amfd_homes_current.farms) as mfd_farm FROM amfd_homes_current amfd_homes_current, amd_temp_pc pclist WHERE amfd_homes_current.PostCode = pclist.POSTCODE and pclist.store_uid = $store_uid";
if ($result = mysqli_query($link, $query)) {
while ($row = mysqli_fetch_assoc($result)) {
$mfd_home = $row['mfd_home'];
$mfd_apts = $row['mfd_apts'];
...
FROM c_weight c_weight, c_ls2 c_ls2, amd_temp_pc pclist WHERE pclist.store_uid = $store_uid and pclist.POSTCODE = c_weight.POSTCODE and pclist.POSTCODE = c_ls2.POSTCODE ";
if ($result = mysqli_query($link, $query)) {
while ($row = mysqli_fetch_assoc($result)) {
...
FROM c_fly c_fly, c_weight c_weight, amd_temp_pc pclist WHERE pclist.store_uid = $store_uid and pclist.POSTCODE = c_fly.POSTCODE and pclist.POSTCODE = c_weight.POSTCODE ";
if ($result = mysqli_query($link, $query)) {
while ($row = mysqli_fetch_assoc($result)) {
$flypp15_ = $row['wPP15_'];
...
,  SUM(c_sp1.D1000_5230 * c_sp1.sp1hh_tot) as aD1000_5230
 ...
 FROM c_sp1 c_sp1, c_sp2 c_sp2, c_sp3 c_sp3, amd_temp_pc pclist WHERE c_sp1.POSTCODE = pclist.POSTCODE AND c_sp2.POSTCODE = pclist.POSTCODE AND c_sp3.POSTCODE = pclist.POSTCODE AND pclist.store_uid = $store_uid ";
		if ($result = mysqli_query($link, $query)) {
			while ($row = mysqli_fetch_assoc($result)) {
...
FROM c_at at1, amd_temp_pc pclist, c_weight c_weight WHERE at1.POSTCODE = pclist.POSTCODE AND c_weight.POSTCODE = pclist.POSTCODE AND pclist.store_uid = $store_uid";
	if ($result = mysqli_query($link, $query)) {
			while ($row = mysqli_fetch_assoc($result)) {
