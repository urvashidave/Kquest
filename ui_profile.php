...
$now = new DateTime(); $change_date = $now->format('F j, Y');
if (isset($_POST["change_prepared"])) {	$change_prepared = $_POST["change_prepared"];} else $change_prepared = "Produced for: " . $user_firstname . " " . $user_lastname . "" . $change_date;
$report_api_key = 3571;
$report_good_yn = 0;
...
if (isset($_POST["change_header"])) {	$change_header = $_POST["change_header"];} 
		...
		$change_header = "Target Audience";
		if($store_uid >= 898 && $store_uid <= 942) $change_header = "Regional Targeting"; 
	}
...
$var_click_details = $var1_fullname;
$var_click_details = str_replace(',', "<br>", $var_click_details);
$var_click_details = rtrim($var_click_details, '<br/>');
$var_click_details = rtrim($var_click_details, ',');
...
if($store_uid >= 898 && $store_uid <= 942) $map_zoom_level = 7;
...
include 'db_connect.php'; 
$saved_vals_yn = 0;
$query = "Select * FROM amfd_savedvals WHERE store_uid = $store_uid and report_uid = $report_uid ";
if ($result = mysqli_query($link, $query)) {
while ($row = mysqli_fetch_assoc($result)) {
...
$need_new_mods_yn = 0;
if($detail_show == 11 || $detail_show == 12) $need_new_mods_yn = 1;	// 1 = need new mods if report is rest or retail

$run_new_mods_yn = 0;
if($new_mods < 1 && $need_new_mods_yn > 0) $run_new_mods_yn = 1;	// 1 = run new mods if needed and data is not saved
...
// the following creates a score for each pc - save temporarily in amfd_temp_scored

if($saved_vals_yn  < 1 || $run_new_mods_yn > 0) {
	...
	$query = "SELECT * FROM $temp_table_name WHERE report_uid = $report_uid AND HH_TOT > 0 ORDER BY (demo_score + bbm_score + sp_score + seg_score + hi_score + homes_score  + auto_score + eth_score) DESC";
		...
		while ($row = mysqli_fetch_assoc($result)) {
			...
			if($decile_counter < 2 && $temp_total_hh < 100000 && $pc_counter < 10000) {
...
// run sum values on target postal codes

	$temp_table_name2 = "amd_temp_reppc";
	$pc_name2 = rtrim($pc_name2, ",");
	mysqli_query($link,"TRUNCATE TABLE $temp_table_name2 ");
	mysqli_query($link,"INSERT INTO $temp_table_name2 (report_uid, POSTCODE) VALUES $pc_name2 ");
	
	include 'old_get_profile.php'; 			// run sum values on target postal codes
	
	// updated august 28 2018 -> amfd_homes to amfd_homes_current
	
	$query = "SELECT 
	SUM(home.homes) as homes, birthyear
	FROM amfd_homes_current home, amd_temp_reppc pclist 
	WHERE pclist.report_uid = $report_uid and pclist.POSTCODE = home.PostCode 
	GROUP BY birthyear";
	if ($result = mysqli_query($link, $query)) {
	while ($row = mysqli_fetch_assoc($result)) {
	...
	for ( $i = 0; $i <= 15; $i ++) {
	if($demo['agehomes'][$i] > 0) $my_temp_list_is_here = 0; else $demo['agehomes'][$i] = 0; 
	}
	...
	$demo['improve_list'] = implode(",", $demo['improve']);
					// new edit on sep 20 - savedvals
	$demo['decile_score_list'] = implode(",", $decile_score_array);
	$demo['agehomes_list'] = implode(",", $demo['agehomes']);
	$demo['PostCodearray_list'] = implode(",", $PostCodearray);
	}
// end run sum values on target postal codes for first time through

 }
include 'db_connect.php'; 
 if($saved_vals_yn < 1) {				// save normal variables
	 
		$temp_val_insert = $demo['summary_list'];
		mysqli_query($link,"INSERT INTO amfd_savedvals (user_uid, company_uid, store_uid, report_uid, agg_name, agg_list) VALUES ($user_uid, $client_id, $store_uid, $report_uid, 'summary', '$temp_val_insert') ");
		$temp_val_insert = $demo['segment_list']; 
		mysqli_query($link,"INSERT INTO amfd_savedvals (user_uid, company_uid, store_uid, report_uid, agg_name, agg_list) VALUES ($user_uid, $client_id, $store_uid, $report_uid, 'segment', '$temp_val_insert') ");
		$temp_val_insert = $demo['media_list']; 
		mysqli_query($link,"INSERT INTO amfd_savedvals (user_uid, company_uid, store_uid, report_uid, agg_name, agg_list) VALUES ($user_uid, $client_id, $store_uid, $report_uid, 'media', '$temp_val_insert') ");
		$temp_val_insert = $demo['mfd_homes_list']; 
		mysqli_query($link,"INSERT INTO amfd_savedvals (user_uid, company_uid, store_uid, report_uid, agg_name, agg_list) VALUES ($user_uid, $client_id, $store_uid, $report_uid, 'mfd_homes', '$temp_val_insert') ");
		$temp_val_insert = $demo['leisure_list']; 
		mysqli_query($link,"INSERT INTO amfd_savedvals (user_uid, company_uid, store_uid, report_uid, agg_name, agg_list) VALUES ($user_uid, $client_id, $store_uid, $report_uid, 'leisure', '$temp_val_insert') ");
		$temp_val_insert = $demo['flyer_list']; 
		mysqli_query($link,"INSERT INTO amfd_savedvals (user_uid, company_uid, store_uid, report_uid, agg_name, agg_list) VALUES ($user_uid, $client_id, $store_uid, $report_uid, 'flyer', '$temp_val_insert') ");
		$temp_val_insert = $demo['spending_list']; 
		mysqli_query($link,"INSERT INTO amfd_savedvals (user_uid, company_uid, store_uid, report_uid, agg_name, agg_list) VALUES ($user_uid, $client_id, $store_uid, $report_uid, 'spending', '$temp_val_insert') ");
		$temp_val_insert = $demo['internet_list']; 
		mysqli_query($link,"INSERT INTO amfd_savedvals (user_uid, company_uid, store_uid, report_uid, agg_name, agg_list) VALUES ($user_uid, $client_id, $store_uid, $report_uid, 'internet', '$temp_val_insert') ");
		$temp_val_insert = $demo['np_list']; 
		mysqli_query($link,"INSERT INTO amfd_savedvals (user_uid, company_uid, store_uid, report_uid, agg_name, agg_list) VALUES ($user_uid, $client_id, $store_uid, $report_uid, 'np', '$temp_val_insert') ");
		$temp_val_insert = $demo['radio_list']; 
		mysqli_query($link,"INSERT INTO amfd_savedvals (user_uid, company_uid, store_uid, report_uid, agg_name, agg_list) VALUES ($user_uid, $client_id, $store_uid, $report_uid, 'radio', '$temp_val_insert') ");
		$temp_val_insert = $demo['tv1_list']; 
		mysqli_query($link,"INSERT INTO amfd_savedvals (user_uid, company_uid, store_uid, report_uid, agg_name, agg_list) VALUES ($user_uid, $client_id, $store_uid, $report_uid, 'tv1', '$temp_val_insert') ");
		$temp_val_insert = $demo['leisure2_list']; 
		mysqli_query($link,"INSERT INTO amfd_savedvals (user_uid, company_uid, store_uid, report_uid, agg_name, agg_list) VALUES ($user_uid, $client_id, $store_uid, $report_uid, 'leisure2', '$temp_val_insert') ");
		$temp_val_insert = $demo['flyer1_list']; 
		mysqli_query($link,"INSERT INTO amfd_savedvals (user_uid, company_uid, store_uid, report_uid, agg_name, agg_list) VALUES ($user_uid, $client_id, $store_uid, $report_uid, 'flyer1', '$temp_val_insert') ");
		$temp_val_insert = $demo['auto_list']; 
		mysqli_query($link,"INSERT INTO amfd_savedvals (user_uid, company_uid, store_uid, report_uid, agg_name, agg_list) VALUES ($user_uid, $client_id, $store_uid, $report_uid, 'auto', '$temp_val_insert') ");
		$temp_val_insert = $demo['improve_list']; 
		mysqli_query($link,"INSERT INTO amfd_savedvals (user_uid, company_uid, store_uid, report_uid, agg_name, agg_list) VALUES ($user_uid, $client_id, $store_uid, $report_uid, 'improve', '$temp_val_insert') ");
				// edit add the 3 new targeting savedvals
				
		$temp_val_insert = $demo['decile_score_list']; 
		mysqli_query($link,"INSERT INTO amfd_savedvals (user_uid, company_uid, store_uid, report_uid, agg_name, agg_list) VALUES ($user_uid, $client_id, $store_uid, $report_uid, 'decile_score', '$temp_val_insert') ");
		$temp_val_insert = $demo['agehomes_list']; 
		mysqli_query($link,"INSERT INTO amfd_savedvals (user_uid, company_uid, store_uid, report_uid, agg_name, agg_list) VALUES ($user_uid, $client_id, $store_uid, $report_uid, 'agehomes', '$temp_val_insert') ");
		$temp_val_insert = $demo['PostCodearray_list']; 
		mysqli_query($link,"INSERT INTO amfd_savedvals (user_uid, company_uid, store_uid, report_uid, agg_name, agg_list) VALUES ($user_uid, $client_id, $store_uid, $report_uid, 'PostCodearray', '$temp_val_insert') ");
	


	mysqli_query($link,"UPDATE amfd_reports SET saved_vals = 1, pc_count = $temp_total_hh WHERE report_uid = $report_uid ");
 } 
 // saved_vals is completed only once - everything is saved and savedvals identifier is updated for the report 
 ...
 if($saved_vals_yn > 0) {
	 ...
	 $query = "Select * FROM amfd_savedvals WHERE store_uid = $store_uid and report_uid = $report_uid ";
		...
		if($row['agg_name'] == "auto") $demo['auto_list'] = $row['agg_list'];
		if($row['agg_name'] == "improve") $demo['improve_list'] = $row['agg_list'];
		if($row['agg_name'] == "agehomes") $demo['agehomes_list'] = $row['agg_list'];
		if($row['agg_name'] == "decile_score") $demo['decile_score_list'] = $row['agg_list'];
		if($row['agg_name'] == "PostCodearray") $demo['PostCodearray_list'] = $row['agg_list'];
		}				
		}
	...
	$demo['improve'] = explode(",", $demo['improve_list']);
					// new edit on sep 20 - savedvals
	$decile_score_array = explode(",", $demo['decile_score_list']);
	$demo['agehomes'] = explode(",", $demo['agehomes_list']);
	$PostCodearray = explode(",", $demo['PostCodearray_list']);
	
	
	// get values if already saved
	// updated august 2018 -> from $demo['segment'][17] to $demo['mfd_homes'][3]


	$total_households = $demo['mfd_homes'][3];
	$decile_cutoff = $total_households * 0.1;
	$average_score_in_area = $decile_score_array[10];
...
if($mod_rest  < 1  && $run_new_mods_yn > 0) {
	...
	$query = "SELECT 
		...
		FROM c_rest rest, amd_temp_reppc pclist, c_weight c_weight WHERE rest.POSTCODE = pclist.POSTCODE AND c_weight.POSTCODE = pclist.POSTCODE AND pclist.report_uid = $report_uid";
...
if($mod_grocery  < 1 && $detail_show == 12) {					//  grocery includes all sh1 - grocery dept (department) cv (convenience drug
	include 'db_connect_wg.php';  
		...
		FROM c_sh1 sh1, amd_temp_reppc pclist, c_weight c_weight WHERE sh1.POSTCODE = pclist.POSTCODE AND c_weight.POSTCODE = pclist.POSTCODE AND pclist.report_uid = $report_uid";
		if ($result = mysqli_query($link, $query)) {
		while ($row = mysqli_fetch_assoc($result)) {
...
if($mod_clothing  < 1 && $detail_show == 12) {					//  grocery includes all sh2 - clothing  shoe
	...
	include 'db_connect_wg.php';  
		...
		FROM c_sh2 sh2, amd_temp_reppc pclist, c_weight c_weight WHERE sh2.POSTCODE = pclist.POSTCODE AND c_weight.POSTCODE = pclist.POSTCODE AND pclist.report_uid = $report_uid";
		if ($result = mysqli_query($link, $query)) {
		while ($row = mysqli_fetch_assoc($result)) {
...
if($mod_book  < 1 && $detail_show == 12) {					//  book includes all sh3 - book jewel furn home
	...
	include 'db_connect_wg.php';  
		...
		FROM c_sh3 sh3, amd_temp_reppc pclist, c_weight c_weight WHERE sh3.POSTCODE = pclist.POSTCODE AND c_weight.POSTCODE = pclist.POSTCODE AND pclist.report_uid = $report_uid";
...
if($mod_eye  < 1 && $detail_show == 12) {					//  eye includes all sh4 - eye, glasses (use spend laser surgery) elect office sport
	...
	include 'db_connect_wg.php';  
		...
		FROM c_sh4 sh4, amd_temp_reppc pclist, c_weight c_weight WHERE sh4.POSTCODE = pclist.POSTCODE AND c_weight.POSTCODE = pclist.POSTCODE AND pclist.report_uid = $report_uid";
...
if($mod_toy  < 1 && $detail_show == 12) {					//  toy includes all sh5 - toy pet pet_own type_r type_o
	...
	include 'db_connect_wg.php';  
		...
		FROM c_sh5 sh5, amd_temp_reppc pclist, c_weight c_weight WHERE sh5.POSTCODE = pclist.POSTCODE AND c_weight.POSTCODE = pclist.POSTCODE AND pclist.report_uid = $report_uid";
...
include 'db_connect.php';
	$demo['rest_list'] = implode(",", $demo['rest']);
		...
		mysqli_query($link,"INSERT INTO amfd_savedvals (user_uid, company_uid, store_uid, report_uid, agg_name, agg_list) VALUES ($user_uid, $client_id, $store_uid, $report_uid, 'rest', '$temp_val_insert') ");
...
include 'db_connect.php';
	$demo['grocery_list'] = implode(",", $demo['grocery']);
		...
		mysqli_query($link,"INSERT INTO amfd_savedvals (user_uid, company_uid, store_uid, report_uid, agg_name, agg_list) VALUES ($user_uid, $client_id, $store_uid, $report_uid, 'grocery', '$temp_val_insert') ");
	$demo['dept_list'] = implode(",", $demo['dept']);
		$temp_val_insert = $demo['dept_list']; 
		mysqli_query($link,"INSERT INTO amfd_savedvals (user_uid, company_uid, store_uid, report_uid, agg_name, agg_list) VALUES ($user_uid, $client_id, $store_uid, $report_uid, 'dept', '$temp_val_insert') ");
	$demo['cv_list'] = implode(",", $demo['cv']);
		$temp_val_insert = $demo['cv_list']; 
		mysqli_query($link,"INSERT INTO amfd_savedvals (user_uid, company_uid, store_uid, report_uid, agg_name, agg_list) VALUES ($user_uid, $client_id, $store_uid, $report_uid, 'cv', '$temp_val_insert') ");
	$demo['drug_list'] = implode(",", $demo['drug']);
		$temp_val_insert = $demo['drug_list']; 
		mysqli_query($link,"INSERT INTO amfd_savedvals (user_uid, company_uid, store_uid, report_uid, agg_name, agg_list) VALUES ($user_uid, $client_id, $store_uid, $report_uid, 'drug', '$temp_val_insert') ");	
...
include 'db_connect.php';
	$demo['clothing_list'] = implode(",", $demo['clothing']);
		...
		mysqli_query($link,"INSERT INTO amfd_savedvals (user_uid, company_uid, store_uid, report_uid, agg_name, agg_list) VALUES ($user_uid, $client_id, $store_uid, $report_uid, 'clothing', '$temp_val_insert') ");
	$demo['shoe_list'] = implode(",", $demo['shoe']);
		$temp_val_insert = $demo['shoe_list']; 
		mysqli_query($link,"INSERT INTO amfd_savedvals (user_uid, company_uid, store_uid, report_uid, agg_name, agg_list) VALUES ($user_uid, $client_id, $store_uid, $report_uid, 'shoe', '$temp_val_insert') ");
}
...
include 'db_connect.php';
	$demo['book_list'] = implode(",", $demo['book']);
		...
		mysqli_query($link,"INSERT INTO amfd_savedvals (user_uid, company_uid, store_uid, report_uid, agg_name, agg_list) VALUES ($user_uid, $client_id, $store_uid, $report_uid, 'book', '$temp_val_insert') ");
	$demo['jewel_list'] = implode(",", $demo['jewel']);
		$temp_val_insert = $demo['jewel_list']; 
		mysqli_query($link,"INSERT INTO amfd_savedvals (user_uid, company_uid, store_uid, report_uid, agg_name, agg_list) VALUES ($user_uid, $client_id, $store_uid, $report_uid, 'jewel', '$temp_val_insert') ");
	$demo['furn_list'] = implode(",", $demo['furn']);
		$temp_val_insert = $demo['furn_list']; 
		mysqli_query($link,"INSERT INTO amfd_savedvals (user_uid, company_uid, store_uid, report_uid, agg_name, agg_list) VALUES ($user_uid, $client_id, $store_uid, $report_uid, 'furn', '$temp_val_insert') ");
	$demo['home_list'] = implode(",", $demo['home']);
		$temp_val_insert = $demo['home_list']; 
		mysqli_query($link,"INSERT INTO amfd_savedvals (user_uid, company_uid, store_uid, report_uid, agg_name, agg_list) VALUES ($user_uid, $client_id, $store_uid, $report_uid, 'home', '$temp_val_insert') ");
}
...
include 'db_connect.php';
	$demo['eye_list'] = implode(",", $demo['eye']);
		...
		mysqli_query($link,"INSERT INTO amfd_savedvals (user_uid, company_uid, store_uid, report_uid, agg_name, agg_list) VALUES ($user_uid, $client_id, $store_uid, $report_uid, 'eye', '$temp_val_insert') ");
	$demo['glasses_list'] = implode(",", $demo['glasses']);
		$temp_val_insert = $demo['glasses_list']; 
		mysqli_query($link,"INSERT INTO amfd_savedvals (user_uid, company_uid, store_uid, report_uid, agg_name, agg_list) VALUES ($user_uid, $client_id, $store_uid, $report_uid, 'glasses', '$temp_val_insert') ");
	$demo['elect_list'] = implode(",", $demo['elect']);
		$temp_val_insert = $demo['elect_list']; 
		mysqli_query($link,"INSERT INTO amfd_savedvals (user_uid, company_uid, store_uid, report_uid, agg_name, agg_list) VALUES ($user_uid, $client_id, $store_uid, $report_uid, 'elect', '$temp_val_insert') ");
	$demo['office_list'] = implode(",", $demo['office']);
		$temp_val_insert = $demo['office_list']; 
		mysqli_query($link,"INSERT INTO amfd_savedvals (user_uid, company_uid, store_uid, report_uid, agg_name, agg_list) VALUES ($user_uid, $client_id, $store_uid, $report_uid, 'office', '$temp_val_insert') ");
	$demo['sport_list'] = implode(",", $demo['sport']);
		$temp_val_insert = $demo['sport_list']; 
		mysqli_query($link,"INSERT INTO amfd_savedvals (user_uid, company_uid, store_uid, report_uid, agg_name, agg_list) VALUES ($user_uid, $client_id, $store_uid, $report_uid, 'sport', '$temp_val_insert') ");
}
...
include 'db_connect.php';
	$demo['toy_list'] = implode(",", $demo['toy']);
		...
		mysqli_query($link,"INSERT INTO amfd_savedvals (user_uid, company_uid, store_uid, report_uid, agg_name, agg_list) VALUES ($user_uid, $client_id, $store_uid, $report_uid, 'toy', '$temp_val_insert') ");
	$demo['pet_list'] = implode(",", $demo['pet']);
		$temp_val_insert = $demo['pet_list']; 
		mysqli_query($link,"INSERT INTO amfd_savedvals (user_uid, company_uid, store_uid, report_uid, agg_name, agg_list) VALUES ($user_uid, $client_id, $store_uid, $report_uid, 'pet', '$temp_val_insert') ");
	$demo['pet_own_list'] = implode(",", $demo['pet_own']);
		$temp_val_insert = $demo['pet_own_list']; 
		mysqli_query($link,"INSERT INTO amfd_savedvals (user_uid, company_uid, store_uid, report_uid, agg_name, agg_list) VALUES ($user_uid, $client_id, $store_uid, $report_uid, 'pet_own', '$temp_val_insert') ");
	$demo['type_r_list'] = implode(",", $demo['type_r']);
		$temp_val_insert = $demo['type_r_list']; 
		mysqli_query($link,"INSERT INTO amfd_savedvals (user_uid, company_uid, store_uid, report_uid, agg_name, agg_list) VALUES ($user_uid, $client_id, $store_uid, $report_uid, 'type_r', '$temp_val_insert') ");
	$demo['type_o_list'] = implode(",", $demo['type_o']);
		$temp_val_insert = $demo['type_o_list']; 
		mysqli_query($link,"INSERT INTO amfd_savedvals (user_uid, company_uid, store_uid, report_uid, agg_name, agg_list) VALUES ($user_uid, $client_id, $store_uid, $report_uid, 'type_o', '$temp_val_insert') ");
}
...
// updated august 28 2018 -> from segment to mfd_homes
$query = "Select * FROM amfd_savedvals WHERE store_uid = $store_uid and report_uid = 0 and agg_name ='segment'";
		if ($result = mysqli_query($link, $query)) {
		while ($row = mysqli_fetch_assoc($result)) {
			$total_households2_list = $row['agg_list'];			
		}
		}
		
$total_households2_array = explode(",", $total_households2_list);
$total_households = $total_households2_array[17];
$decile_cutoff = $total_households * 0.1;

// updated august 28 2018 -> from segment to mfd_homes		
$query = "Select * FROM amfd_savedvals WHERE store_uid = $store_uid and report_uid = 0 and agg_name ='mfd_homes'";
		if ($result = mysqli_query($link, $query)) {
		while ($row = mysqli_fetch_assoc($result)) {
			$total_households2_list_mfd_homes = $row['agg_list'];			
		}
		}
		$total_households2_list_mfd_homes_array = explode(",", $total_households2_list_mfd_homes);
$total_households = $total_households2_list_mfd_homes_array[3];
$decile_cutoff = $total_households * 0.1;


//$short_header = substr($var1_fullname_string,0, 40) . " ...";	
$short_header = $var1_fullname_string;
$var1_fullname_string = rtrim(rtrim($var1_fullname_string, "<br>"), "<br/>");
$var1_fullname_string = str_replace(',', '<br>', $var1_fullname_string);
...
$query = "Select lat, lon FROM amfd_homes_current WHERE PostCode = '$PostCodearray[$count_dwi]' ";
if ($result = mysqli_query($link, $query)) {
while ($row = mysqli_fetch_assoc($result)) {
...
$count_dwi_1 = $count_dwi-1;
$temp_lastarray = $cust_toronto[$count_dwi_1];
$cust_toronto[$count_dwi_1] = rtrim($temp_lastarray, ",");
...
dodocument.getElementById('store_selected').value = val;
var val1 = document.getElementById("myReport").value;
document.getElementById('detail_show').value = val1;
var val2 = document.getElementById("myBench").value;
document.getElementById('rbenchmark').value = val2;
...
var i1 = "<?php echo $allindex[0];?>";
   	var i2 = "<?php echo $allindex[1];?>";
   	var i3 = "<?php echo $allindex[2];?>";
   	var i4 = "<?php echo $allindex[3];?>";
   	var i5 = "<?php echo $allindex[4];?>";
   	var i6 = "<?php echo $allindex[5];?>";
   	var i7 = "<?php echo $allindex[6];?>";
   	var i8 = "<?php echo $allindex[7];?>";
   	var i9 = "<?php echo $allindex[8];?>";
   	var i10 = "<?php echo $allindex[9];?>";
   	var i11 = "<?php echo $allindex[10];?>";
   	var i12 = "<?php echo $allindex[11];?>";
   	var i13 = "<?php echo $allindex[12];?>";
   	var i14 = "<?php echo $allindex[13];?>";
   	var i15 = "<?php echo $allindex[14];?>";
   	var i16 = "<?php echo $allindex[15];?>";
   	var i17 = "<?php echo $allindex[16];?>";
   function detail_show3() {	
   window.open("segment_desc.php?i1="+i1+"&i2="+i2+"&i3="+i3+"&i4="+i4+"&i5="+i5+"&i6="+i6+"&i7="+i7+"&i8="+i8+"&i9="+i9+"&i10="+i10+"&i11="+i11+"&i12="+i12+"&i13="+i3+"&i14="+i14+"&i15="+i15+"&i16="+i16+"&i17="+i17+"");
   }



function detail_show2() {
   	window.open("segment_desc.php?i1="+i1+"&i2="+i2+"&i3="+i3+"&i4="+i4+"&i5="+i5+"&i6="+i6+"&i7="+i7+"&i8="+i8+"&i9="+i9+"&i10="+i10+"&i11="+i11+"&i12="+i12+"&i13="+i3+"&i14="+i14+"&i15="+i15+"&i16="+i16+"&i17="+i17+"");
   
   }
function detail_show4() {
...
var chart = new google.visualization.AreaChart(document.getElementById('chart_div4b'));
chart.draw(data4b, options4b);
}
google.load("visualization", "1", {packages:["corechart"]});
google.setOnLoadCallback(drawChart);
function drawChart() { 
...
<div class="noprint pro">
<div id="page-wrap">
<form action="" method="post">
<div align='center'>  
<table width="100%" border="0" cellspacing="0" cellpadding="0">
<tr style="border-bottom: 2px solid black;">
<td width="40%" class="black18" valign="bottom"><strong>
...
<br />
<input name="change_title_but1" type="submit" class="bg_color2 target-red" id="change_title_but1"  onClick="TITLEsubmit2(<?php echo $store_uid ; ?>);" value="Change Title" style="margin-bottom: 5px;"/></strong>
</td>
<td width="20%" valign="bottom"><input name="roi_but1" type="submit" class="roi_but1 target-red" id="roi_but1"  onClick="CALCREPORTsubmit(<?php echo $report_uid * 3571; ?>);" value="ROI Calculator" style="margin-bottom: 15px;"/>
</strong></td>
<td width="40%" align="right" valign="bottom"><p>
<select id="myBench" class=""  onchange="list_bench()">
...
<p>
<select id="myReport" class=""  onchange="list_report();document.forms[0].submit()">
...
<br />
<br />
<input name="change_report" type="submit" class="bg_color2 target-red" id="change_report"  onClick="change_report(<?php echo $store_uid ; ?>);" value="Update Report" style="margin-bottom: 5px;"/>
</p></td>
</tr>
</table>
</div> 
...
<div id="proc_div" class="" style="display: none;"><img src="please_wait.gif" width="572" height="304" alt="processing"></div>
</div>
</div>
<?php 

/// selecvt the agg anbd pages to view 
if ($detail_show == 0) { include 'profile_cover2.php'; include 'ui_psummarytop.php';  include 'ui_psummary.php';  }
// demographics
if ( $detail_show == 2) {  include "ui_detail_cover.php";   echo "<div class='pagebreak'></div>"; echo "<div class='page-wrap'>";  include "ui_detail2p.php"; echo "</div"; }
// spending 
if ( $detail_show == 4) {  include "ui_detail_cover.php";   echo "<div class='pagebreak'></div>"; echo "<div class='page-wrap'>";  include "ui_detail4p.php"; echo "</div"; }
// Media 
if ( $detail_show == 5) {  include "ui_detail_cover.php";   echo "<div class='pagebreak'></div>"; echo "<div class='page-wrap'>";  include "ui_detail5p.php"; echo "</div"; }
// lifestyle
if ( $detail_show == 6) {  include "ui_detail_cover.php";   echo "<div class='pagebreak'></div>"; echo "<div class='page-wrap'>";  include "ui_detail6p.php"; echo "</div"; }
// Automotive
if ( $detail_show == 7) {  include "ui_detail_cover.php";   echo "<div class='pagebreak'></div>"; echo "<div class='page-wrap'>";  include "ui_detail8p.php"; echo "</div"; }
// iMPROVE
if ( $detail_show == 8) {  include "ui_detail_cover.php";   echo "<div class='pagebreak'></div>"; echo "<div class='page-wrap'>";  include "ui_detailhhp.php"; echo "</div"; }

if ( $detail_show == 11) {  echo "<div class='page-wrap'>"; include "ui_detail_rest_p.php"; echo "</div"; }
//if ( $detail_show == 12) { include "ui_detail_grocery_ta.php";  }
if ( $detail_show == 12) {  echo "<div class='page-wrap'>"; include "ui_detail_retail_p.php"; echo "</div"; }
...
.btn-primary: hover{\
background-color:  #b21254  !important; \
}";
style.appendChild(document.createTextNode(styleText));
document.body.appendChild(style);
//return style.sheet;
console.log("Ran style sheet updater");
return;
})();
</script>
</html>
